...
def putImpl[T, K <: Tuple, V](map: Expr[CMap[String, V]], value: Expr[V])(using
    Quotes,
    Type[T],
    Type[K],
    Type[V]
): Expr[Unit] =
  opImpl[T, K, V, Unit](map, '{ ${ map }.put(${ Expr(Type.show[T]) }, ${ value }) })

def opImpl[T, K <: Tuple, V, Result](map: Expr[CMap[String, V]], res: Expr[Result])(using
    Quotes,
    Type[T],
    Type[K],
    Type[V]
